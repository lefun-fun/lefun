...
import type { GameDef, GameStateBase, GetPayloadOf } from "@lefun/game";
...
type MakeMove<G extends GameDef<any, any>> = <
...
type MakeMoveFull<G extends GameDef<any, any>> = <
  K extends keyof G["playerMoves"] & string,
>(
  name: K,
  ...payload: IfNever<
    GetPayloadOf<G["playerMoves"][K]>,
    [GetPayloadOf<G["playerMoves"][K]> | undefined],
    [GetPayloadOf<G["playerMoves"][K]>]
  >
) => void;
...
// FIXME `setMakeMove` if we go in that direction!
export function setMakeMove(
  makeMove: (store: Store) => MakeMoveFull<GameDef<any, any>>,
) {
  _makeMove = makeMove;
}

type GetGameStatesFromGameDef<G extends GameDef<any, any>> =
  G extends GameDef<infer GS, any> ? GS : never;

export function useMakeMove<G extends GameDef<any, any>>(): MakeMove<G> {
  ...
  const store: Store<GetGameStatesFromGameDef<G>> | null =
...
export function makeUseMakeMove<G extends GameDef<any, any>>() {
