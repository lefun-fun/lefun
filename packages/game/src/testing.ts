...
type MatchTesterOptions<
  G extends GameStateBase,
  PM extends PlayerMoveDefs<G, BMT>,
  BM extends BoardMoveDefs<G, BMT>,
  BMT,
> = {
  gameDef: GameDef<G, PM, BM, BMT>;
  gameData?: any;
  matchData?: any;
  numPlayers: number;
  numBots?: number;
...
// type SpecialMoves = 'lefun/initMove' | 'lefun/addPlayerMove' | 'lefun/kickPlayer';

// type MoveKeys<M extends Record<string, PlayerMove<G>>> = keyof M extends string

// type MoveNames<K> = K | SpecialMoves;

// type Move<G extends GameType, M extends Record<string, PlayerMove<G>>> = {
// name: keyof M extends string ? keyof M : never; // | SpecialMoves;
//   payload: M[keyof M];
// };
...
export class MatchTester<
  ...
  BM extends BoardMoveDefs<GS, BMT> = BoardMoveDefs<GS, any>,
  BMT = EmptyObject,
> {
  gameDef: GameDef_<GS, PM, BM, BMT>;
  gameData: any;
  matchData?: any;
  meta: Meta;
  board: GS["B"];
  ...
  delayedMoves: DelayedMove<BMT, any>[];
  ...
  constructor({
    ...
    locale = "en",
  }: MatchTesterOptions<GS, PM, BM, BMT>) {
    if (random == null) {
      random = new Random();
    }
  ...
  makeSpecialExecuteFuncs() {
    ...
    // FIXME any
    const delayMove = <K extends BoardMoveName<BMT>>(
      move: BoardMoveWithOptionalPayload<BMT, K>,
      delay: number,
    ) => {
      const dm = delayedMove<BMT, K>(move, this.time + delay);
  ...
  makeBoardMove<K extends BoardMoveName<BMT>>(
    move: BoardMoveWithOptionalPayload<BMT, K>,
  ) {
  ...
  async makeNextBotMove() {
    ...
    for (
      ...
      if (isBot && itsYourTurn) {
        ...
        // FIXME
        let autoMoveRet: AutoMoveRet<GS, PM> | AgentGetMoveRet<GS, PM>;
        ...
        // FIXME
        let move: Move<any, any> | undefined;
  ...
  fastForward(delta: number): void {
    ...
    delayedMoves
      .map((u, i) => [u, i] as [DelayedMove<BMT, any>, number])
