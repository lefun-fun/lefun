...
export type BMTBase = Record<string, unknown>;
...
export type SpecialFuncs<BMT = BMTBase> = {
  delayMove: <K extends keyof BMT & string>(
    name: K,
...
// `any` doesn't seem to work but `infer A` does.
export type GetPayloadOf<PM> =
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  PM extends PlayerMoveDef<infer A, infer P, infer B> ? P : never;
...
export type GameDef<
  ...
  BMT extends BMTBase = EmptyObject,
  PM extends Record<string, PlayerMoveDef<GS, any, BMT>> = any,
  BM extends Record<string, BoardMoveDef<GS, any, BMT>> = any,
> = {
 ...
 */
export type GameDef_<GS extends GameStateBase, BMT extends BMTBase> = Omit<
  GameDef<GS, BMT>,
  "stats"
> & {
  stats?: {
 ...
 */
export function parseGameDef<GS extends GameStateBase, BMT extends BMTBase>(
  gameDef: GameDef<GS, BMT>,
): GameDef_<GS, BMT> {
  // Normalize the stats.
  const { stats: stats_ } = gameDef;

  const stats: GameDef_<GS, BMT>["stats"] = {
    allIds: [],
    byId: {},
  };
